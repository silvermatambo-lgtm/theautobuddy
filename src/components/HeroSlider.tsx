...
const heroText='Making Vehicle Buying Simple';

export default function HeroSlider(){
  const[typed,setTyped]=useState('');
  useEffect(()=>{
    const delay=typed.length<heroText.length?75:1800;
    const timer=window.setTimeout(()=>{
      setTyped(current=>current.length<heroText.length?heroText.slice(0,current.length+1):'');
    },delay);
    return()=>window.clearTimeout(timer);
  },[typed]);

  const firstLine=typed.slice(0,14);
  const secondLine=typed.slice(14).trimStart();
    ...
    <div className="relative z-10 mx-auto grid min-h-[690px] max-w-[1600px] grid-cols-1 px-4 py-8 lg:grid-cols-[330px_1fr] lg:px-6">
      <aside className="relative z-20 hidden lg:block">
        <div className="sticky top-28">
          <img src="/images/autobuddy-logo-final.svg?v=font20260902" alt="The Autobuddy" className="-ml-4 w-[360px] max-w-none drop-shadow-2xl"/>
      ...
      <div className="relative pl-0 lg:pl-5">
        <div className="flex min-h-[650px] flex-col justify-center">
          <div className="max-w-[950px]">
            ...
            <h1 className="autobuddy-title min-h-[1.9em] text-5xl leading-[.9] text-white sm:text-6xl md:text-7xl lg:text-[86px]">
              <span className="typewriter-line">{firstLine}</span><br/>
              <span className="typewriter-line text-[#f7b500]">{secondLine}</span><span className="typewriter-cursor">|</span>
            </h1>
            <p className="mt-5 max-w-2xl text-lg font-bold italic text-slate-200">We inspect. We value. We negotiate. You buy with confidence.</p>
            <div className="mt-5 flex flex-wrap gap-4 text-xs font-black uppercase italic tracking-widest text-white">{['Inspected','Valued','Verified'].map(x=><span key={x} className="flex items-center gap-2 rounded-full bg-white/10 px-3 py-2"><CheckCircle2 size={17} className="text-[#19d35b]"/>{x}</span>)}</div>
            <div className="mt-7 grid max-w-[880px] gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <a className="flex items-center justify-center gap-2 rounded-xl bg-[#13b84b] px-4 py-3 text-sm font-black uppercase italic text-white shadow-xl" href="https://wa.me/27824080680"><MessageCircle size={18}/> Chat on WhatsApp</a>
              <Link className="flex items-center justify-center gap-2 rounded-xl bg-[#f7b500] px-4 py-3 text-sm font-black uppercase italic text-[#061a35] shadow-xl" to="/catalogue"><LockKeyhole size={18}/> Trade Catalogue</Link>
              <Link className="flex items-center justify-center gap-2 rounded-xl bg-[#0870dc] px-4 py-3 text-sm font-black uppercase italic text-white shadow-xl" to="/upload-details"><Upload size={18}/> Upload Vehicle</Link>
              <Link className="flex items-center justify-center gap-2 rounded-xl bg-[#ff6a00] px-4 py-3 text-sm font-black uppercase italic text-white shadow-xl" to="/contact"><ClipboardCheck size={18}/> Request Inspection</Link>
            </div>
          </div>
          ...
          <div className="relative mt-7 hidden min-h-[250px] md:block">
            ...
            <div className="absolute left-0 top-7 z-20 w-[43%] rounded-3xl bg-white p-4 text-[#061a35] shadow-2xl autobuddy-glow">
              <div className="grid grid-cols-2 gap-2 text-xs font-black italic sm:grid-cols-4">
                {['Toyota Hilux','BMW X3','VW Polo GTI','Ford Ranger'].map(x=><div key={x} className="rounded-xl bg-[#eef7ff] p-3"><span className="mb-1 block text-[10px] text-green-600">AVAILABLE</span>{x}<span className="mt-1 block text-[#0870dc]">View Details</span></div>)}
              </div>
            </div>
          </div>
        </div>
